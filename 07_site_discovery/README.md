...
The filters evaluated during our parameter sweep primarily target base and mapping qualities of individual reads. These filters remove the lowest-quality data before calculating genotype likelihoods,
...
where is the observed sequencing data and is a candidate genotype at locus . However, such filters cannot resolve cases where the data do not in fact originate from a unique, real locus .

Our next objective was therefore to identify sites that are plausibly generated by a single genomic locus. While the existence of cannot be tested directly, we can construct variables that summarize aspects of the sequencing data expected to differ between _real_ and _spurious_ loci.

ANGSD provides several commonly used diagnostic metrics, including:
...
While widely applied, these statistics are not particularly powerful indicators of spurious loci (see [HTSlib filtering guidelines](https://www.htslib.org/workflow/filter.html)), and their sensitivity is likely further reduced by the combined challenges of this dataset: a large genome, short-read sequencing, and an imperfect reference assembly.
